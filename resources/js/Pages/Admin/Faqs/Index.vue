...
<template>
    <AppLayout title="Preguntas Frecuentes">
        <template #header>
            <h2 class="font-slab text-xl text-tinta leading-tight">Preguntas Frecuentes</h2>
        </template>
        ...
        <div class="py-6">
            ...
            <div v-if="faqs && faqs.length > 0" class="space-y-3">
                <div v-for="(faq, i) in faqs" :key="faq.id"
                    class="bg-white rounded-2xl border border-tinta/10 p-5 hover:shadow-sm transition-shadow">
                    <div class="flex items-start gap-4">
                        <!-- Reorder arrows -->
                        <div class="flex flex-col gap-0.5 pt-0.5">
                            <button @click="moveUp(i)" :disabled="i === 0"
                                class="p-0.5 text-tinta/30 hover:text-tinta disabled:opacity-20 transition-colors" :aria-label="'Subir ' + faq.question">
                                <ChevronUpIcon class="w-4 h-4" />
                            </button>
                            <button @click="moveDown(i)" :disabled="i >= faqs.length - 1"
                                class="p-0.5 text-tinta/30 hover:text-tinta disabled:opacity-20 transition-colors" :aria-label="'Bajar ' + faq.question">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-3 mb-1">
                                <h3 class="font-slab text-tinta truncate">{{ faq.question }}</h3>
                                <StatusBadge :status="faq.is_published ? 'approved' : 'rejected'" variant="gallery" />
                            </div>
                            <p class="text-tinta/50 text-sm line-clamp-2">{{ faq.answer }}</p>
                        </div>

                        <div class="flex items-center gap-1 flex-shrink-0">
                            <button @click="togglePublished(faq)" class="p-2 text-tinta/30 hover:text-primary transition-colors rounded-lg hover:bg-primary/5" :title="faq.is_published ? 'Ocultar' : 'Publicar'">
                            ...
                            <button @click="openEditFaq(faq)" class="p-2 text-tinta/30 hover:text-primary transition-colors rounded-lg hover:bg-primary/5" title="Editar">
                            ...
                            <ConfirmDeleteModal :message="`¿Eliminar la pregunta «${faq.question.substring(0, 50)}...»?`" @confirm="doDelete">
                                <template #default="{ open: openDel }">
                                    <button @click="confirmDelete(faq); openDel()" class="p-2 text-tinta/30 hover:text-red-500 transition-colors rounded-lg hover:bg-red-50" title="Eliminar">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea v-model="faqForm.answer" rows="4"
                                class="w-full mt-1 rounded-xl border-tinta/20 focus:border-primary focus:ring-primary/20"
                        ...
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <span class="text-sm text-tinta/70">Mostrar en el sitio público</span>
                        </label>
                    </div>
                </template>
                <template #footer>
